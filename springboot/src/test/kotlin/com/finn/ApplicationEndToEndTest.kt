...
class ApplicationEndToEndTest {
    ...
    fun `full user community post lifecycle succeeds`() {
        ...
        val communityResponse = postJson(
            url = "/communities",
            payload = CommunityDto(
                id = null,
                title = "E2E",
        ...
        val postResponse = postJson(
            url = "/posts",
            payload = PostDto(id = null, content = "content", image = null, userId = "user-e2e", communityId = communityId),
        )
        val postId = postResponse["id"].asLong()

        val commentResponse = postJson(
            url = "/comments",
            payload = CommentDto(id = null, content = "great", userId = "user-e2e", postId = postId),
        )
        val commentId = commentResponse["id"].asLong()

        val updatedComment = CommentDto(id = commentId, content = "even better", userId = "user-e2e", postId = postId)
        mockMvc.perform(
        ...
        val likeResponse = mockMvc.perform(
            post("/posts/likes")
                .param("user_id", "user-e2e")
        ...
        val communityWithImage = CommunityDto(id = null, title = "E2EImage", description = "With image", image = null, userId = "user-e2e")
        val communityMultipart = multipart("/communities")
            .file(MockMultipartFile("community", "community.png", MediaType.IMAGE_PNG_VALUE, ByteArray(32) { 1 }))
            .param("community", objectMapper.writeValueAsString(communityWithImage))
        val communityMultipartResult = mockMvc.perform(communityMultipart)
            .andExpect(status().isCreated)
        ...
        val postWithImage = PostDto(id = null, content = "content with image", image = null, userId = "user-e2e", communityId = communityId)
        val postMultipart = multipart("/posts")
            .file(MockMultipartFile("post", "post.png", MediaType.IMAGE_PNG_VALUE, ByteArray(48) { 2 }))
            .param("post", objectMapper.writeValueAsString(postWithImage))
        val postMultipartResult = mockMvc.perform(postMultipart)
            .andExpect(status().isCreated)
            .andExpect(jsonPath("$.image").isNotEmpty)
            .andReturn()
        ...
        val subscribersBefore = mockMvc.perform(get("/communities/$communityId/subscribers"))
            .andExpect(status().isOk)
            .andReturn()
        ...
        val subscribersAfter = mockMvc.perform(get("/communities/$communityId/subscribers"))
            .andExpect(status().isOk)
            .andReturn()
        assertEquals("0", subscribersAfter.response.contentAsString)
    }

    private fun postJson(url: String, payload: Any): JsonNode {
